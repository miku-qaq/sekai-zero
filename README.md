...
Mikureina 长期维护、持续迭代的动漫主题个人网站。首页以明亮的「白昼动画杂志」作为世界入口，用个人简介、当前学习、世界地图、三位喜欢的角色频道与次元扭蛋帮助访客快速认识 Mikureina；详细内容沿「关于我、奇妙收藏馆、学习笔记、导航终端、日志」五条清晰路线展开。奇妙收藏馆把动画、游戏、手办与 Fufu 毛绒收进同一座馆：动画分馆忠实整理 89 部已看作品，游戏分馆从本地 Steam 记录生成 146 条可维护的公开目录，手办分馆记录 16 件已经核对角色与作品出处的收藏，其中 7 件为初音未来，Fufu 分馆记录大 Fufu 与寅、卯、辰、巳、午五件生肖款，共计 257 条真实记录。重点不是堆砌功能，而是建立稳定的工程地基、鲜明但克制的视觉系统，以及可以安全扩展的内容边界。
...
Fufu 分馆另外记录寅、卯、辰、巳、午五件生肖款。六件 Fufu 的正式商品名均以对应 SEGA 商品页为准，并逐字保留官方原文的空格、半角或全角括号与 `LL` / `ＬＬ`，不为了视觉统一擅自改写标点。生肖数量、首尾年份与时间线都从 `content/fufu.ts` 的实际记录派生，未来新增生肖不需要同步修改页面里的固定数字。商品图片仍优先等待 Mikureina 本人实拍，并在加入 `public/figures/` 或 `public/fufu/` 前记录来源、替代文本与优化规格。
