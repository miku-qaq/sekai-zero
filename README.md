...
Mikureina 长期维护、持续迭代的动漫主题个人网站。首页以明亮的「白昼动画杂志」作为世界入口，用个人简介、当前学习、世界地图、三位喜欢的角色频道与次元扭蛋帮助访客快速认识 Mikureina；详细内容沿「关于我、动画收藏、游戏收藏、学习笔记、导航终端、日志」六条清晰路线展开。动画收藏馆忠实整理 Mikureina 提供的已看片单，游戏收藏馆则从本地 Steam 记录生成可维护的公开目录，并为两类封面保留清楚的来源与版权边界。重点不是堆砌功能，而是建立稳定的工程地基、鲜明但克制的视觉系统，以及可以安全扩展的内容边界。
...
2. 身份、文案、动画与游戏清单、学习内容和导航终端收藏优先修改 `content/`；每一话统一登记在 `content/releases.ts`，当前学习与兴趣信号维护在 `content/now.ts`，页面组合放在 `app/`。
