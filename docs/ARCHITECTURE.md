...
学习笔记目前使用强类型数据，每篇统一保留知识主线、最小示例、易错边界、主动回忆与一手资料；课程类内容还要区分“正在自学公开资料”、正式选课与课程完成，并优先使用课程官方一手来源。带笔记锚点的 URL 会恢复筛选、展开目标并定位正文。是否迁移到构建期 MDX 和 `/study/[slug]/` 由实际维护成本、文章长度与检索需求决定，不设置任意篇数门槛；运行时仍不得读取本地 MDX 文件。出现更多长文章后，通过 `getPosts()` 一类稳定查询接口提供给页面。需要在线编辑、留言或用户数据时，再在查询接口下接 Cloudflare D1；媒体上传出现后再启用 R2。
