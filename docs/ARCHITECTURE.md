...
站点级内容放在 `content/site.ts`；主人确认公开的身份与联系字段只在 `content/profile.ts` 维护，About 负责渲染完整资料，Links 只链接到该联系入口，避免内容漂移和无意义重复。角色兴趣文案、航线、制作档案、学习笔记和日志分别放在 `content/about.ts`、`content/links.ts`、`content/projects.ts`、`content/study.ts` 与 `content/logs.ts`，让每周文案更新不触碰布局。制作档案只使用能够从仓库或主人资料核对的目标、职责、过程和结果；没有证据的项目、数字或评价不得补位。学习笔记目前使用强类型数据，每篇统一保留知识主线、最小示例、易错边界、主动回忆与一手资料；课程类内容还要区分“正在自学公开资料”、正式选课与课程完成，并优先使用课程官方一手来源。超过约 8–10 篇后迁移到构建期 MDX 和 `/study/[slug]/`，而不在 Worker 运行时读取文件。出现更多长文章后，通过 `getPosts()` 一类稳定查询接口提供给页面。需要在线编辑、留言或用户数据时，再在查询接口下接 Cloudflare D1；媒体上传出现后再启用 R2。
