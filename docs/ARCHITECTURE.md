...
站点级内容放在 `content/site.ts`，个人档案、航线和日志分别放在 `content/about.ts`、`content/links.ts` 与 `content/logs.ts`，让每周文案更新不触碰布局。出现长文章后引入构建期 MDX 管线，并通过 `getPosts()` 一类稳定查询接口提供给页面。需要在线编辑、留言或用户数据时，再在查询接口下接 Cloudflare D1；媒体上传出现后再启用 R2。
